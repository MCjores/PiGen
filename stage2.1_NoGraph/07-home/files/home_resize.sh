...
get_variables () {
  ...
  HOME_PART_DEV=$(findmnt /home -o source -n | cut -d "[" -f 1)
  HOME_PART_NAME=$(echo "$HOME_PART_DEV" | cut -d "/" -f 3)
  HOME_DEV_NAME=$(echo /sys/block/*/"${HOME_PART_NAME}" | cut -d "/" -f 4)
  HOME_PART_NUM=$(cat "/sys/block/${HOME_DEV_NAME}/${HOME_PART_NAME}/partition")
  ...
  HOME_PART_LINE=$(echo "$PARTITION_TABLE" | grep -e "^${HOME_PART_NUM}:")
  HOME_PART_START=$(echo "$HOME_PART_LINE" | cut -d ":" -f 2)
  HOME_PART_END=$(echo "$HOME_PART_LINE" | cut -d ":" -f 3)
...
check_variables () {
  ...
  if [ "$HOME_PART_NUM" -ne "$LAST_PART_NUM" ]; then
    FAIL_REASON="Home partition should be last partition"
    return 1
  fi

  if [ "$HOME_PART_END" -gt "$TARGET_END" ]; then
    FAIL_REASON="Home partition runs past the end of device"
    return 1
  fi

  if [ ! -b "$ROOT_DEV" ] || [ ! -b "$HOME_PART_DEV" ] || [ ! -b "$ROOT_PART_DEV" ] || [ ! -b "$BOOT_PART_DEV" ] ; then
...
main () {
  ...
  if [ "$HOME_PART_END" -eq "$TARGET_END" ]; then
    reboot_pi
  fi
  ...
  if ! parted -m "$ROOT_DEV" u s resizepart "$HOME_PART_NUM" yes "$TARGET_END"; then
    FAIL_REASON="Home partition resize failed"
    return 1
  fi
...
sed -i 's| init=/usr/lib/home_resize.sh||' /boot/cmdline.txt
...
if main; then
  whiptail --infobox "Resized home filesystem. Rebooting in 5 seconds..." 20 60
  sleep 5
else
  sleep 5
